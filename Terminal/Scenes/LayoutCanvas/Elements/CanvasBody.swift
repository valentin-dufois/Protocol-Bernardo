...
class CanvasBody: SKShapeNode {
	...
	convenience init(forBody body: Pb_Network_Messages_Body) {
		...
		xLabel = SKLabelNode(text: String(format: "%.2f", -body.skeletons.last!.centerOfMass.x / 10.0))
		...
		yLabel = SKLabelNode(text: String(format: "%.2f", body.skeletons.last!.centerOfMass.z / 10.0))
